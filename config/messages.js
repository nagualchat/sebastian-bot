const messages = {
    ...
    deleteDel2 : 'Сообщение участника $username было удалено, потому это $reason. Чтобы я больше такого не видел, понятно?',
    ...
    ban1 : '$username изгоняется из чата без возможности вернуться обратно.',
    ban2 : '$username изгоняется из чата. Он может попробовать вернуться через $duration.',
    ...
    favAddCapt : 'Необходимо указать название для закладки в таком виде: `/fav название`. Оно может содержать несколько слов, но в целом должно быть не длиннее 50 символов.',
    ...
    '`/ban (длительность) (причина)` : выгоняет и банит участника.\n' + 
