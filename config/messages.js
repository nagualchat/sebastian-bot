const messages = {
    ...
    '`/dels 123,456 (причина)` : удаляет несколько сообщений по ID (должны быть перечислены без пробелов).\n' +
    '`/kick (причина)` : выгоняет участника из чата.\n' + 
    '`/ban (причина)` : выгоняет и банит участника. Длительность бана не может быть указана из-за недоработок библиотеки!\n' + 
    ...
    'Команды кроме `/say` и `/dels` должны применяться к процитированному сообщению. Параметры в скобках опциональны. Длительность указывается вида `1h` или `1d`, что соответствует 1 часу и 1 дню. Если не указана, то считается бесконечной.'
  };
